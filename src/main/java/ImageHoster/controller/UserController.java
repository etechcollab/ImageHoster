...
public class UserController {
    ...
    //http://www.regular-expressions.info/java.html for reference of REGEX Concept and how it is used for page validation.
...
//Password validation can also be done using the REGEX Concept that by using Redirect reattr and by using compile keyword.
    //here I have implemented using boolean method using array concept

    public static boolean passwordValidation(String password) {
        String[] verifyExpression = {".*[a-zA-Z]+.*",
                ".*[0-9]+.*",
                ".*[!@#$%^&*(),.?:{}|<>]+.*"
        };

        boolean CheckPassword = (password.matches(verifyExpression[0]) && (password.matches(verifyExpression[1]) && password.matches(verifyExpression[2])));
